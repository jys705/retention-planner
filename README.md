...
![적어둔 항목 하나의 기억 곡선. 복습할 때마다 100%로 올라갔다가 잊는 곡선을 따라 내려오고, 가운데 '다시'를 고른 자리에서 기억 지속력이 무너져 간격이 짧아진다. 앞으로 잡힌 일정은 점선으로 목표한 날까지 이어진다.](docs/screenshot.png)

복습은 앱 밖에서 합니다. 원래 진행하던 공부를 마친 뒤에 돌아와서
자가 평가 네 단계(다시, 어려움, 무난함, 쉬움) 중 하나만 고르면 다음 복습일이 다시 잡힙니다.

![오늘 화면. 오늘 볼 항목 3개가 목록으로 서 있고, 줄마다 지금 기억률과 소속 목표가 붙어 있다. 맨 아래 한 줄로 새 항목을 적는 칸이 있다.](docs/today.png)

목표 시점을 정해두면 그날 목표 기억률을 넘도록 일정을 역산하고,
하루에 몰리지 않게 날짜를 펴서 잡습니다.
해야 하는 복습은 늦어도 목표한 날 전날까지 끝나도록 잡고,
목표한 날 당일에 남는 것은 해도 되고 안 해도 되는 것뿐입니다.

![예보 화면. 앞으로 38일치 복습이 막대 그래프로 서 있고 목표한 날마다 세로선이 그어져 있다. 대략으로 잡은 목표는 옅은 띠로 구간을 보여준다. 아래에는 같은 내용을 달력으로 본 그림이 있다.](docs/forecast.png)
