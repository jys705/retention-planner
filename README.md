...
![적어둔 항목 하나의 기억 곡선. 복습할 때마다 100%로 올라갔다가 잊는 곡선을 따라 내려온다. 가운데 '다시'를 고른 자리에서 기억 지속력이 무너져 간격이 짧아지고, 앞으로 잡힌 일정은 점선으로 목표한 날까지 이어진다.](docs/memory-curve.png)

개인적으로 하던 공부를 마치고 돌아와서 네 단계 (다시, 어려움, 무난함, 쉬움)
중 하나를 선택해 자가 평가하면, 다음에 복습할 날짜가 계산됩니다.

![오늘 화면. 오늘 볼 항목 셋이 목록으로 서 있고 줄마다 지금 기억률과 소속 목표가 붙어 있다. 아래에는 새 항목을 적는 칸이 상세 설정과 함께 펼쳐져 있고, 오른쪽에 앞으로 언제 보게 될지가 미리 나온다.](docs/today-screen.png)

목표하는 날이 존재하거나 복습 강도를 따로 정하고 싶을 때는 상세 설정을 사용합니다.
