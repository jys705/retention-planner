...
describe('서재', () => {
  it('S-070 목표별로 묶어 보여준다', async () => {
    ...
    // 바깥 묶음 머리글이 소속 목표다.
    expect(
      await screen.findByRole('button', { name: 'AWS SCS-C03' })
    ).toBeInTheDocument()
  ...
  it('S-071 목표 묶음을 풀어 한 줄로 본다', async () => {
    ...
    expect(
      screen.getByRole('button', { name: '목표별로 묶기' })
    ).toBeInTheDocument()
  })

  it('S-072 정렬: 다음에 볼 날', async () => {
    await setupApp(TODAY, mixed())
    const { user } = render(
      <LibraryScreen onOpenItem={noop} onOpenGoal={noop} />
    )
  ...
  it('S-078 제목 묶음이 목표처럼 보이지 않는다', async () => {
    await setupApp(TODAY, mixed())
    render(<LibraryScreen onOpenItem={noop} onOpenGoal={noop} />)
    await screen.findByRole('button', { name: '목표 없음' })

    // 제목이 비슷해서 접어둔 묶음에는 '제목' 표시가 붙는다.
    const stemHeader = screen.getByRole('button', {
      name: '제목 묶음 정보보안 개념',
    })
    expect(within(stemHeader).getByText('제목')).toBeInTheDocument()
    expect(stemHeader.getAttribute('title')).toMatch(/목표 소속과는 다릅니다/)

    // 바깥 묶음은 소속 목표이고 설명이 붙는다.
    expect(
  ...
  it('S-080b 목표 이름을 눌러 목표 상세로 간다', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'AWS SCS-C03' }))
